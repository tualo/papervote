...
CREATE TABLE IF NOT EXISTS `wahlschein` (
  ...
  `abgabetyp` varchar(255) DEFAULT 0,
...
CREATE OR REPLACE TRIGGER `trigger_wahlschein_bi_defaults`
...
CREATE OR REPLACE TRIGGER `trigger_wahlschein_bu_defaults`
BEFORE UPDATE ON `wahlschein` FOR EACH ROW
BEGIN
  IF NEW.login IS NULL THEN
    SET NEW.login = getSessionUser();
  END IF;
