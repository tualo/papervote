...
CREATE OR REPLACE VIEW `view_readtable_import_ausgangszustand` AS (
    select
        ...
        `wahlscheinstatus`.`ridx` AS `new_state_ridx`,
        `x`.`name` AS `before_state_name`,
        `x`.`ridx` AS `before_state_ridx`
    from
        (
            (
...
OR REPLACE VIEW `view_readtable_wahlschein_by_identnr` AS (
    select
        ...
        `b`.`ridx` AS `wahlberechtigte_ridx`,
...
OR REPLACE VIEW `view_readtable_import_status_loop` AS (
    select
        ...
        `view_readtable_wahlschein_by_identnr`.`wahlberechtigte_ridx` AS `wahlberechtigte_ridx`,
        ...
        (
            (
                ...
                join `view_readtable_counter_wahlscheinstatus` on(
                    `view_readtable_counter_wahlscheinstatus`.`wahlberechtigte` = `view_readtable_wahlschein_by_identnr`.`wahlberechtigte_ridx`
                )
            )
            join `wahlscheinstatus` on(
                `wahlscheinstatus`.`ridx` = `view_readtable_wahlschein_by_identnr`.`wahlscheinstatus`
            )
        )
) //
...
OR REPLACE VIEW `view_read_table_impoprt_status_proc_loop` AS (
    select
        ...
        `iaz`.`new_state_ridx` AS `new_state_ridx`
    from
        (
            (
                (
            ...
            join `view_readtable_import_ausgangszustand` `iaz` on(
                ...
                and `iaz`.`before_state_ridx` = `isl`.`wahlscheinstatus`
            )
        )
) //
...
OR REPLACE VIEW `view_read_table_impoprt_status_proc_final_loop` AS (
    select
        ...
        `view_read_table_impoprt_status_proc_loop`.`new_state_ridx` AS `new_state_ridx`,
        if(
