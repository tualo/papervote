...
Ext.define('Tualo.PaperVote.lazy.counting.Logic', {
  ...
  getBallotpaper: function(code) {
    ...
    var res = this.stimmzettelgruppen.findRecord('ridx',code,0,false,false,true);
    if (Ext.isEmpty(res)){
      return false;
    }
    return res;
    ...
    try {
      ...
      switch (signal) {
              ...
              for (var i = r.length - 2; i >= 0; i--) {
                if (r[i].get('typ') == 3) {
                  klist.push({
                    ...
                    kandidatridx: r[i].get('kandidatridx'),
                    //allowed: this.allowedSZbySZRidx(r[i].get('stimmzettel'))
                  });
        ...
        default:
          ...
          if ((this.FSM.getState() == 'initStack'))  {this.setMessage('Stimmzettelanfang erwartet');
          ...
          if (
            ...
            if(this.checkHasCandidate(candidateres.get('ridx'))){
              this.FSM.transit('candidate');
              var itm = this.store.add({
                stapel: this.stapel,
                stimmzettelnr: this.stimmzettelnr,
                ...
                code: code,
                kandidat: code,
                kandidatridx: candidateres.get('ridx'),
  ...
  checkBallotPaper: function(stimmzettel){
    ...
    if (me.currentBP===null){
      ...
      me.stimmzettelgruppen.getRange().forEach(function(rec){
        ...
        me.currentBPGroups.push(rec.get('ridx'));
      });
  ...
  checkHasCandidate: function(ridx){
    var me = this;
    ...
    for(var i=range.length-1;i>0;i--){
      ...
      if ( (range[i].get('typ')=='3') ){
        if ( (range[i].get('kandidatridx')==ridx) ){
  ...
  checkCurrentBallotPaper: function(stimmzettelgruppe){
    ...
    this.stimmzettelgruppen.getRange().forEach(function(rec){
      if (rec.get('stimmzettel')==me.currentBP){
        checkHash[rec.get('ridx')]=rec.get('sitze')*1;
        nameHash[rec.get('ridx')]=rec.get('name');
      }
    });
    ...
    var szres = this.stimmzettel.findRecord('ridx',me.currentBP,0,false,false,true);
