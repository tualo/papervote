Ext.define('Tualo.PaperVote.lazy.monitor.controller.SZGViewport', {
  extend: 'Ext.app.ViewController',
  alias: 'controller.cmp_wm_wahlausschuss_mon_szg',
  ...
  onMonitorStoreLoad: function (b, records, successful, operation, eOpts) {
    ...
    if (records != null)
      if (records.length > 0) {
        this.lookupReference('chart').captions.title.setText(records[0].get('stimmzettelgruppen_name'));
  ...
  onStimmzettelSelect: async function (rm, record, index, eOpts) {
    ...
    this.getViewModel().getStore('view_stimmenanzahl_ranking_los_monitor').filter([
      {
        property: 'stimmzettelgruppen_id',
        operator: 'eq',
        value: record.get('stimmzettelgruppen_id')
      }
    ]);
