            ...
            try {
                ...
                foreach ($liste as $wert) {
                    ...
                    if (isset($system_settings['remote-erp/url'])  && strlen($system_settings['remote-erp/url']['property']) > 6) {
                        ...
                        App::logger('SAVE')->debug($system_settings['remote-erp/url']['property'] . '/~/' . $system_settings['remote-erp/token'] . '/onlinevote/get/' . $loadWS['id']);
                        $online_result = WMTualoRequestHelper::query($system_settings['remote-erp/url']['property'] . '/~/' . $system_settings['remote-erp/token'] . '/onlinevote/get/' . $loadWS['id']);
